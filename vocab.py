...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_in_char_idxs=self.words2charindices(sents)
        sents_padded=pad_sents_char(sents_in_char_idxs, 0)
        max_sentence_length=len(sents_padded[0]) 
        max_word_length=len(sents_padded[0][0])
